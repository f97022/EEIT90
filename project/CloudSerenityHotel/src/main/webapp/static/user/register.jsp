...
<body>
	...
	<div class="container ">
		<form method="post" action="../user/register">
			...
			</select> <br> <label for="email" class="form-label fs-5">電子信箱</label> <input
				type="email" class="form-control w-25" id="email" name="email"
				placeholder="請輸入電子信箱" required> <br> <label
				for="password" class="form-label fs-5">密碼</label> <input
				type="password" class="form-control w-25" id="password"
				name="password" placeholder="請輸入密碼" minlength="8" maxlength="64"
				required> <span id="passwordMsg" style="visibility: hidden;"><i
				class="fa-solid fa-circle-xmark"></i> </span> <br> <label for="name"
				class="form-label fs-5">姓名</label> <input type="text"
				class="form-control w-25" id="name" name="name" placeholder="請輸入姓名"
				required> <br> <label for="gender"
				class="form-label fs-5">性別</label> <select class="form-select w-25"
				id="gender" name="gender" required>
				<option value="Male">男</option>
				<option value="Female">女</option>
				<option value="Other">其他</option>
			</select> <br> <label for="birthday" class="form-label fs-5">生日</label> <input
				type="date" class="form-control w-25" id="birthday" name="birthday"
				required> <br> <label for="phone"
				class="form-label fs-5">行動電話</label> <input type="tel"
				class="form-control w-25" id="phone" name="phone"
				placeholder="請輸入行動電話" required> <br> <label
				for="personal-id-no" class="form-label fs-5">身分證字號</label> <input
				type="text" class="form-control w-25" id="personal-id-no"
				name="personal_id_no" placeholder="請輸入身分證字號" required> <span
				id="twIdMsg" style="visibility: hidden;"><i
				class="fa-solid fa-circle-xmark"></i> 身分證字號錯誤</span> <br> <label
				for="country" class="form-label fs-5">國家</label> <select
				class="form-select w-25" id="country" name="country" required>

			</select> <br>
			<div class="form-group"></div>
			<label for="address" class="form-label fs-5">聯絡地址</label> <input
				type="text" class="form-control w-50" id="address" name="address"
				minlength="10" placeholder="請輸入地址" required> <br> <label
				for="passport-no" class="form-label fs-5">護照號碼</label> <input
				type="text" class="form-control w-25" id="passport-no"
        ...
        fetch('https://restcountries.com/v3.1/all')
            ...
            .then(data => {
                ...
                data.forEach(country => {
                    ...
                    if (country.cca3 === 'TWN') { // 讓預設選中的選項是台灣
